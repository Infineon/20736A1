...
#	print "args: @ARGS\n";
	...
	# help for 20736
	$section_lut->{CODE_AREA} = $section_lut->{RO_AREA}
			if defined $section_lut->{RO_AREA}->{sh_addr} && !defined $section_lut->{CODE_AREA}->{sh_addr};
	$section_lut->{SRAM_AREA} = $section_lut->{RW_AREA}
			if defined $section_lut->{RW_AREA}->{sh_addr} && !defined $section_lut->{SRAM_AREA}->{sh_addr};
...
sub output_ld
{
    ...
    die "Could not locate CODE_AREA or RO_AREA in patch elf\n" if !defined $rom_start;
