...
// Allocates a block of memory for Bluetooth LE Tx or Rx use.  Tx is for data to be
// transmitted over the air.  Rx is for data to be sent to the host.  The
// payloadSize parameter does not include the size of an ACL header.  The
// returned buffer will be of sufficient size of store both the four byte ACL
// header and the requested payload size.  This function may only be called from
...
typedef enum
{
    CFA_UII_BT_DEVICE_DESC = 0,         // Bluetooth device descriptor
    CFA_UII_BT_CONFIG_DESC = 1,         // Bluetooth configuration descriptor
...
#endif // CFA_USB_SUPPORTED

#ifdef CFA_TRAN_SUPPORTED
//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
// 10.7 Transport
//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
...
// Retuns the remote wake status for the Bluetooth port
