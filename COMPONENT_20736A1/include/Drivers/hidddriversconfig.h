...
typedef PACKED struct
{
    ...
    /// See "MUX_CONFIG Defines Section" in gpiodriver.h for full description.
    UINT16 ctrl;
    /// GPIO IO configuration for all bits not determined by ctrl value.
    UINT16 gpioValue;
} MuxConfigEntry;
...
typedef PACKED struct
{
    /// Spiffy clk line mux configuration values.
    MuxConfigEntry clk;

    /// Spiffy miso line mux configuration values.
    MuxConfigEntry miso;

    /// Spiffy mosi line mux configuration values.
    MuxConfigEntry mosi;

    /// Spiffy chip select line mux configuration values.
    /// Only applicable to spiffy2 slave.
    MuxConfigEntry cs;
} SpiffyConfig;
